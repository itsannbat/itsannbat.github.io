...
export function createMenubar(props) {
    ...
    const menubar = builder(name(), {
        ...
        action: (node) => {
            ...
            if (!menuTriggers.length || !isHTMLElement(menuTriggers[0]))
                return {};
            menuTriggers[0].tabIndex = 0;
            const menus = Array.from(node.querySelectorAll('[data-melt-menubar-menu]')).filter((el) => isHTMLElement(el));
     ...
     */
    function handleCrossMenuNavigation(e, activeMenu) {
        if (!isBrowser)
            return;
        e.preventDefault();
